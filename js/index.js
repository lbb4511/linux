...
$(function () {
  ...
  function loadContent() {
    ...
    $.get(CONFIG.path + page + ".md", function (data) {
      var urls = page.split("/")
      if (urls.length > 0) {
        marked.setOptions({ baseUrl: urls.slice(0, urls.length - 1).join('') + "/" });
      }
  ...
  $.get(CONFIG.path + CONFIG.summaryMd, function (data) {
  ...
  // home
  if (CONFIG.logo) {
    $(".book-header-logo").attr('href', CONFIG.logo);
  } else {
    $(".book-header-logo").remove();
  }
